<template>
    <DashboardLayout currentPage="cost">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            <StatCard label="This Month" :sublabel="currentMonth" icon="attach_money" borderColor="primary" valueColor="white">
                <span class="font-mono">${{ totalThisMonth.toLocaleString() }}</span>
            </StatCard>
            <StatCard label="Forecast" sublabel="MONTH-END ESTIMATE" icon="trending_up" borderColor="secondary">
                <span class="font-mono text-secondary">${{ (totalThisMonth * 1.1).toLocaleString(undefined, {maximumFractionDigits: 0}) }}</span>
            </StatCard>
            <StatCard label="Daily Average" sublabel="BASED ON 30 DAYS" icon="analytics" borderColor="tertiary">
                <span class="font-mono text-tertiary">${{ (totalThisMonth / 30).toLocaleString(undefined, {maximumFractionDigits: 2}) }}</span>
            </StatCard>
        ...
        <section class="bg-surface/50 backdrop-blur-sm rounded-lg border border-white/5 p-8 mb-10 relative overflow-hidden">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                <div class="flex flex-wrap items-center gap-4">
                    <span v-for="service in services" :key="service.name" class="flex items-center gap-2 px-2.5 py-1 bg-white/5 rounded-full border border-white/5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                <div v-for="service in services.slice(0, 10)" :key="service.name" class="p-4 bg-surface-elevated/40 rounded-lg border border-white/5 hover:bg-surface-elevated/60 transition-all flex flex-col justify-between overflow-hidden">
                    ...
                    <div class="text-lg font-mono font-bold text-white">${{ service.thisMonth.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse min-w-[800px]">
                    <thead class="bg-canvas text-[10px] font-mono text-slate-400 uppercase tracking-[0.2em] border-b border-white/5">
                        <tr>
                            ...
                            <th class="px-6 py-4 font-normal">This Month</th>
                            <th class="px-6 py-4 font-normal">Last Month</th>
                            <th class="px-6 py-4 font-normal">Change</th>
                            <th class="px-6 py-4 font-normal text-right">Percentage</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-white/5">
                        <tr v-for="service in services" :key="service.name" class="group hover:bg-white/[0.03] transition-colors">
                            <td class="px-6 py-5">
                            ...
                            <td class="px-6 py-5">
                                <span class="font-mono text-sm text-white">${{ service.thisMonth.toFixed(2) }}</span>
                            </td>
                            <td class="px-6 py-5">
                                <span class="font-mono text-sm text-content-variant opacity-70">${{ service.lastMonth.toFixed(2) }}</span>
                            </td>
                            <td class="px-6 py-5">
                                <div :class="[
                        ...
                        <tr v-if="services.length === 0">
                            <td colspan="5" class="px-6 py-12 text-center text-content-variant font-mono text-xs italic">No cost data available for this month.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
const topServiceName = computed(() => {
    if (props.services.length === 0) return 'None';
    return props.services[0].name;
});

const topServiceLabel = computed(() => {
    if (props.services.length === 0) return '$0 (0%)';
    const top = props.services[0];
    return `$${top.thisMonth.toLocaleString()} (${top.pct}%)`;
});
