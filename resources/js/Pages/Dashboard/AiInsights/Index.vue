<template>
    <DashboardLayout currentPage="ai-insights">
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
            <div>
                <h1 class="text-4xl lg:text-5xl font-display font-bold tracking-tight text-slate-900 mb-2">AI Insights</h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            <StatCard label="Recommendations" :value="String(pendingCount)" sublabel="PENDING REVIEW" icon="auto_awesome" borderColor="primary" />
            <StatCard label="Potential Savings" sublabel="IF ALL APPLIED" icon="savings" borderColor="tertiary" />
            <StatCard label="Applied" :value="String(appliedCount)" sublabel="ALL TIME" icon="check_circle" borderColor="secondary" />
        ...
        <div class="space-y-6">
            <div v-for="rec in recommendations" :key="rec.id"
                 :class="[
                     'bg-white rounded-xl border border-slate-900 p-6 relative overflow-hidden transition-all group hover:bg-slate-50 shadow-ambient',
                     rec.status !== 'pending' ? 'opacity-60 grayscale-[0.2]' : 'hover:-translate-y-0.5'
                 ]">
                <div class="flex flex-col lg:flex-row gap-8 relative z-10">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-slate-600 text-sm mb-4 font-sans leading-relaxed max-w-3xl">{{ rec.description }}</p>
                        <div class="flex flex-wrap items-center gap-6 text-[11px] font-mono text-slate-500">
                            <div class="flex items-center gap-2 px-2.5 py-1 bg-slate-50 rounded-md border border-slate-200">
                                <span class="material-symbols-outlined text-[14px] text-primary">data_object</span>
                                <span><span class="text-slate-900 font-bold opacity-60">RESOURCE</span>: {{ rec.resource_id }}</span>
                            </div>
                            <div class="flex items-center gap-2 px-2.5 py-1 bg-slate-50 rounded-md border border-slate-200">
                                <span class="material-symbols-outlined text-[14px] text-slate-400">settings_applications</span>
                                <span><span class="text-slate-900 font-bold opacity-60">ACTION</span>: <span class="text-slate-900">{{ rec.action_type }}</span></span>
                            </div>
                        </div>
                        ...
                        <div v-if="rec.status === 'pending'" class="flex items-center gap-3 mt-6">
                            ...
                            <button @click="dismissRecommendation(rec.id)"
                                class="bg-white hover:bg-slate-50 text-slate-600 border border-slate-300 px-5 py-2 rounded-lg font-mono text-[10px] uppercase tracking-widest transition-all active:scale-95">
                                Dismiss
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex lg:flex-col gap-3 lg:gap-3 shrink-0 lg:items-end">
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200 min-w-[130px] shadow-sm group-hover:border-primary/20 transition-colors">
