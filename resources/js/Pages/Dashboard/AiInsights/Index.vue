    ...
    <DashboardLayout currentPage="ai-insights">
        ...
        <div class="space-y-6">
            <div v-for="rec in recommendations" :key="rec.id"
                 :class="[
                     'bg-surface/50 backdrop-blur-md rounded-xl border border-white/5 p-8 relative overflow-hidden transition-all group hover:bg-surface-elevated/40 shadow-sm',
                     rec.status !== 'pending' ? 'opacity-60 grayscale-[0.2]' : 'shadow-glass shadow-primary/5 hover:-translate-y-1'
                 ]">
                <div class="flex flex-col lg:flex-row gap-8 relative z-10">
                    <!-- Icon -->
                    <div :class="[
                        'w-14 h-14 flex items-center justify-center rounded-2xl shrink-0 border border-white/10 shadow-glass group-hover:scale-110 transition-transform duration-300',
                        actionIconBg(rec.action_type)
                    ]">
                        <span :class="['material-symbols-outlined text-[28px]', actionIconColor(rec.action_type)]">{{ actionIcon(rec.action_type) }}</span>
                    </div>
 
                    <!-- Content -->
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-4 mb-3">
                            <h3 :class="['font-display text-xl font-bold tracking-tight', rec.status === 'dismissed' ? 'line-through text-slate-500' : 'text-white group-hover:text-primary transition-colors']">{{ rec.title }}</h3>
                            <span :class="[
                                'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono border uppercase tracking-[0.2em] font-bold',
                                statusBadge(rec.status)
                            ]">
                                {{ rec.status }}
                            </span>
                        </div>
                        <p class="text-slate-400 text-base mb-6 leading-relaxed max-w-3xl">{{ rec.description }}</p>
                        ...
                        <div v-if="rec.status === 'pending'" class="flex items-center gap-3 mt-8">
                            <button @click="applyRecommendation(rec.id)"
                                class="bg-primary/20 hover:bg-primary/30 text-primary border border-primary/30 px-6 py-2 rounded-lg font-mono text-[11px] uppercase tracking-widest font-bold transition-all active:scale-95 shadow-glow shadow-primary/10">
                                Apply Implementation
                            </button>
                            <button @click="dismissRecommendation(rec.id)"
                                class="bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white border border-white/10 px-6 py-2 rounded-lg font-mono text-[11px] uppercase tracking-widest transition-all active:scale-95">
                                Dismiss
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex lg:flex-col gap-4 lg:gap-4 shrink-0 lg:items-end">
                        <div class="bg-canvas/60 backdrop-blur-md p-4 rounded-xl border border-white/10 min-w-[140px] shadow-glass group-hover:border-primary/20 transition-colors">
                            <div class="text-[9px] font-mono text-slate-500 uppercase tracking-[0.2em] mb-2 opacity-70">Confidence</div>
                            <div class="text-2xl font-display font-bold text-white tracking-tighter">{{ rec.confidence_score }}%</div>
                            <div class="w-full h-1.5 bg-white/5 mt-3 rounded-full overflow-hidden border border-white/5">
                        ...
                        <div class="bg-canvas/60 backdrop-blur-md p-4 rounded-xl border border-white/10 min-w-[140px] shadow-glass group-hover:border-tertiary/20 transition-colors">
                            <div class="text-[9px] font-mono text-slate-500 uppercase tracking-[0.2em] mb-2 opacity-70">Savings</div>
                            <div class="text-2xl font-display font-bold text-tertiary tracking-tighter">${{ rec.estimated_monthly_saving }}<span class="text-xs text-slate-500 ml-1 font-mono opacity-60">/mo</span></div>
                        </div>
                    </div>
                </div>
