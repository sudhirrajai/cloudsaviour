        ...
        <div class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div v-for="stat in statsCards" :key="stat.label" 
                    class="bg-surface border border-white/5 p-6 rounded-2xl shadow-glass hover:border-primary/30 transition-all group overflow-hidden relative">
                    <!-- Decorative Glow -->
                    <div :class="['absolute -right-4 -top-4 w-24 h-24 rounded-full blur-3xl opacity-10 group-hover:opacity-20 transition-opacity', stat.glowColor]"></div>
                    ...
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            <p class="text-xs font-mono text-slate-500 uppercase tracking-widest">{{ stat.label }}</p>
                            <h3 class="text-2xl font-display font-bold text-white">{{ stat.value }}</h3>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 bg-surface border border-white/5 rounded-2xl shadow-glass overflow-hidden">
                ...
                <div class="bg-surface border border-white/5 rounded-2xl shadow-glass overflow-hidden">
                    ...
                    <div class="p-8 space-y-6">
                        <div v-for="plan in subscriptionBreakdown" :key="plan.plan" class="space-y-2">
                            <div class="flex justify-between items-end">
                                <span class="text-sm font-bold text-white capitalize">{{ plan.plan }}</span>
                        ...
                        <div class="mt-10 pt-6 border-t border-white/5 grid grid-cols-2 gap-4">
                            <div class="flex flex-col gap-1">
                                ...
                                <span class="text-sm font-bold text-white">{{ topPlan }}</span>
                            </div>
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] text-slate-500 uppercase tracking-widest font-mono">Conversion</span>
                                <span class="text-sm font-bold text-tertiary">{{ conversionRate }}%</span>
                            </div>
                        </div>
                    </div>
                </div>
...
const statsCards = computed(() => [
    ...
    { 
        ...
        icon: 'star', 
        iconBg: 'bg-tertiary shadow-tertiary/20', 
